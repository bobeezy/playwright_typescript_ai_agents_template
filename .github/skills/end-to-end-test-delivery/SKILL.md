...
Description: Deliver complete Playwright automation for a requirement spanning web UI, API, or both. Use when a single ticket or flow needs coverage across multiple layers.
---
...
4. **Implement by layer** in this order:
   - **Web UI** → `../browser-page-objects/SKILL.md` for page objects, then write `tests/web/*.web.spec.ts`
   - **API** → `../api-test-development/SKILL.md` for clients and `tests/api/*.api.spec.ts`
...
| Responsive / device | `mobile-test-development` | `playwright.config.ts` project + `tests/web/` |
...
- Web specs: import `test`, `expect` from `fixtures/webTest.ts`
- API specs: import `test`, `expect` from `fixtures/apiTest.ts`
...
| Scope | Command |
|-------|---------|
| New web spec only | `npx playwright test tests/web/<file>.web.spec.ts` |
| New API spec only | `npx playwright test tests/api/<file>.api.spec.ts` |
